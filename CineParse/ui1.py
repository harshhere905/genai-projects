import streamlit as st
from langchain_google_genai import ChatGoogleGenerativeAI
from dotenv import load_dotenv
from langchain_core.prompts import ChatPromptTemplate

load_dotenv()

st.set_page_config(page_title="Movie Info Extractor", page_icon="🎬")

st.title("🎬 Movie Info Extractor")
st.caption("Paste a paragraph containing movie details, and extract structured info from it.")

# Initialize model once
@st.cache_resource
def get_model():
    return ChatGoogleGenerativeAI(
        model="gemini-2.5-flash",
        temperature=0.5,
    )

model = get_model()

# Build the extraction prompt once
@st.cache_resource
def get_prompt():
    return ChatPromptTemplate.from_messages([
        ("system", """You are an expert movie information extractor. 
Given a paragraph that may contain information about one or more movies, 
extract all the essential details mentioned. Do not assume or invent 
information that isn't present in the text — only extract what's explicitly stated.

Extract the following details wherever available:
- Movie Name
- Release Year
- Genre
- Director
- Cast (main actors/actresses)
- Rating (IMDb, critics, or any mentioned)
- Plot Summary (brief, 1-2 lines)
- Language
- Box Office Collection (if mentioned)
- Awards (if mentioned)

If any detail is not present in the paragraph, simply skip it — do not write "N/A" or make guesses.

Format your output strictly as Markdown, using this exact structure for each movie:

### Movie Name: <name>
- **Release Year:** ...
- **Genre:** ...
- **Director:** ...
- **Cast:** ...
- **Rating:** ...
- **Plot Summary:** ...
- **Language:** ...
- **Box Office:** ...
- **Awards:** ...

If the paragraph mentions multiple movies, repeat this structure for each movie 
(one heading + bullet list per movie), separated by a blank line."""),

        ("human", "{paragraph}")
    ])

extraction_prompt = get_prompt()

# Sample paragraph for quick testing
sample_para = """Christopher Nolan's Inception, released in 2010, is a mind-bending sci-fi thriller starring 
Leonardo DiCaprio, Joseph Gordon-Levitt, and Elliot Page. The film explores dream manipulation 
and has an IMDb rating of 8.8. It grossed over $836 million worldwide and won 4 Academy Awards, 
including Best Visual Effects. On the other hand, 3 Idiots, a 2009 Bollywood comedy-drama 
directed by Rajkumar Hirani, features Aamir Khan, R. Madhavan, and Sharman Joshi in lead roles. 
The movie, which critiques the Indian education system, holds an IMDb rating of 8.4 and became 
one of the highest-grossing Bollywood films of its time, earning around ₹460 crore globally. 
It was shot primarily in Hindi and received widespread critical acclaim along with several 
Filmfare Awards."""

# Sidebar
with st.sidebar:
    st.header("Options")
    if st.button("Load Sample Paragraph"):
        st.session_state.para_input = sample_para

# Text area for input
if "para_input" not in st.session_state:
    st.session_state.para_input = ""

paragraph = st.text_area(
    "Paste your movie paragraph here:",
    value=st.session_state.para_input,
    height=250,
    placeholder="e.g. Christopher Nolan's Inception, released in 2010, is a sci-fi thriller starring..."
)

extract_btn = st.button("Extract Movie Info", type="primary")

if extract_btn:
    if not paragraph.strip():
        st.warning("Please enter a paragraph first.")
    else:
        with st.spinner("Extracting details..."):
            final_prompt = extraction_prompt.invoke({"paragraph": paragraph})
            response = model.invoke(final_prompt)

        st.subheader("Extracted Details")
        # Ensure single newlines aren't collapsed by markdown rendering
        formatted = response.content.replace("\n", "  \n")
        st.markdown(formatted)